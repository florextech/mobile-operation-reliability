...
/** Mobile signals are hints only: neither network nor lifecycle transitions operations. */
...
export function connectLifecycle(scheduler: OperationScheduler): Unsubscribe {
  const stopNetwork = scheduler.start();
  const appState = AppState.addEventListener('change', state => { if (state === 'active') void scheduler.run(); });
  return () => { stopNetwork(); appState.remove(); };
}

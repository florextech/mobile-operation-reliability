...
export default function App() {
  const [engine, setEngine] = useState<OperationEngine | null>(null); const [scheduler, setScheduler] = useState<OperationScheduler | null>(null); const [history, setHistory] = useState<string[]>([]);
  useEffect(() => { void bootstrap().then(value => { setEngine(value.engine); setScheduler(value.scheduler); setHistory(['Storage opened; the lab is ready.']); }).catch(error => setHistory([`Bootstrap failed: ${String(error)}`])); }, []);
  const accept = async () => {
    if (!engine) return;
    ...
    setHistory(previous => [`${handle.id}: ACCEPTED (durable local record)`, ...previous]); await scheduler?.run(); const status = await handle.status(); setHistory(previous => [`${handle.id}: ${status.kind === 'OK' ? status.value?.status : status.error.code}`, ...previous]);
  };
...
async function bootstrap(): Promise<{ engine: OperationEngine; scheduler: OperationScheduler }> {
  ...
  let sequence = 0; const transport = createLabTransport({ baseUrl: process.env.EXPO_PUBLIC_FIXTURE_URL ?? 'http://127.0.0.1:3000' });
  ...
  const scheduler = new OperationScheduler(engine, { principalScope: 'lab-user', targetScope: 'local' }, { scheduleWake: () => () => {} }); return { engine, scheduler };
}

...
async function bootstrap(): Promise<LabRuntime> {
  ...
  const network = await ExpoNetworkHint.open(); let sequence = 0; const runId = `${Date.now()}-${Math.random().toString(36).slice(2)}`; const transport = createLabTransport({ baseUrl: process.env.EXPO_PUBLIC_FIXTURE_URL ?? 'http://127.0.0.1:3000' });

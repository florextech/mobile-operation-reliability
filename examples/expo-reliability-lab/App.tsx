...
export default function App() {
  ...
  return <SafeAreaView style={styles.screen}><ScrollView contentContainerStyle={styles.content}>
    ...
    <View style={styles.listHeader}><Text style={styles.sectionTitle}>Operation timeline</Text><Text style={styles.count}>{rows.length}</Text></View>
    {rows.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>No operations yet</Text><Text style={styles.emptyText}>Create a payment to inspect its durable status and event timeline.</Text></View> : rows.map(row => <OperationCard key={row.id} row={row} />)}
...
function OperationCard({ row }: { readonly row: OperationRow }) { const tone = row.status in statusTone ? styles[statusTone[row.status as OperationStatus]] : styles.failed; return <View style={styles.operationCard}><View style={[styles.statusPill, tone]}><Text style={styles.statusText}>{row.status}</Text></View><Text style={styles.operationId} numberOfLines={1}>{row.id}</Text><Text style={styles.eventText}>{row.events} durable {row.events === 1 ? 'event' : 'events'}</Text></View>; }
